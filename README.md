...
You need two values: `SITE_PASSWORD` (what visitors type in) and
`SESSION_SECRET` (an internal random string used to sign the login cookie —
it doesn't need to be memorable). The Worker code supports **either** of the
two ways Cloudflare lets you set these — use whichever is easier for you.

**Option A — CLI (`wrangler secret put`):**
...
**Option B — Dashboard (Secrets Store):**

In the Cloudflare dashboard, go to your Worker → **Settings → Bindings →
Add**, choose **Secrets Store**, and add bindings named exactly
`SITE_PASSWORD` and `SESSION_SECRET` (variable name must match exactly —
that's how the Worker code looks them up). Create the secret values there if
they don't already exist in your store.

You can mix and match — e.g. set `SITE_PASSWORD` via the dashboard and
`SESSION_SECRET` via the CLI. The code checks for either kind automatically.

Anyone visiting the site will need `SITE_PASSWORD` to get in.
